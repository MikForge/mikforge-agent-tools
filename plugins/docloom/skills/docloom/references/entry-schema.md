...
不定义 feedback_note 的 Header 格式、文件命名规则和整体结构。
...
- `specid`：触发规则编号。spec 文档内部规则 ID（如 `DSG-problem-statement`、`BSG-reference-links`、`PRD-user-story-format`）；无法精确匹配到单条规则时填 `uncategorized`
...
- **specid:** ASG-description-min-length
- **location:** 描述:12
